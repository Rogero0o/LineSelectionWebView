...
android.selection.selectionChanged = function(){
	...
	try{
		...
		var jsonArray = "[";
		var objRectList = range.getClientRects(),  length = objRectList.length;

	    for (var i=0; i<length; i++) {
	    	var rectObj = {};
	    	rectObj.top = objRectList[i].top;
	    	rectObj.right = objRectList[i].right
	    	rectObj.bottom = objRectList[i].bottom;
	    	rectObj.left = objRectList[i].left;
	    	rectObj.width = objRectList[i].width;
	    	rectObj.height = objRectList[i].height;
	    	var jsonText = JSON.stringify(rectObj);
	    	if(i < length -1){
	    		jsonArray += jsonText + ","
	    	}else{
	    		jsonArray += jsonText + "]";
	    ...
	    window.TextSelection.jsError("longTouch - " + jsonArray);
	    window.TextSelection.selectionClientRects(jsonArray,window.getSelection().toString());    
